...
class KatanaGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Project Katana - Game Automation Tool")
    ...
    def create_widgets(self):
        ...
        self.notebook.add(self.workflow_tab, text="Workflow Editor")
    ...
    def setup_main_tab(self):
        ...
        # Game list
        ttk.Label(game_frame, text="Select a game:").grid(row=0, column=0, sticky=tk.W, pady=5)
        
        self.game_listbox = tk.Listbox(game_frame, height=6)
        self.game_listbox.grid(row=1, column=0, columnspan=2, sticky=tk.W+tk.E+tk.N+tk.S, pady=5)
        
        # Add scrollbar to game list
        game_scrollbar = ttk.Scrollbar(game_frame, orient=tk.VERTICAL, command=self.game_listbox.yview)
        game_scrollbar.grid(row=1, column=2, sticky=tk.N+tk.S)
        ...
        # Game details
        ttk.Label(game_frame, text="Game details:").grid(row=2, column=0, sticky=tk.W, pady=5)
        
        self.details_text = tk.Text(game_frame, height=4, wrap=tk.WORD)
        self.details_text.grid(row=3, column=0, columnspan=2, sticky=tk.W+tk.E, pady=5)
        ...
        ttk.Button(button_frame, text="Refresh Games", command=self.refresh_games).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Test Components", command=self.test_components).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Check Game Status", command=self.check_game_status).pack(side=tk.LEFT, padx=5)
        
        # Create workflow frame
        workflow_frame = ttk.LabelFrame(main_frame, text="Workflow Control", padding=10)
        workflow_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        # Workflow buttons
        wf_button_frame = ttk.Frame(workflow_frame)
        wf_button_frame.pack(fill=tk.X, pady=10)
        
        self.start_button = ttk.Button(wf_button_frame, text="Start Workflow", command=self.start_workflow)
        self.start_button.pack(side=tk.LEFT, padx=5)
        
        self.launch_button = ttk.Button(wf_button_frame, text="Launch Game Only", command=self.launch_game)
        self.launch_button.pack(side=tk.LEFT, padx=5)
        
        self.close_button = ttk.Button(wf_button_frame, text="Close Game", command=self.close_game)
        self.close_button.pack(side=tk.LEFT, padx=5)
        
        self.stop_button = ttk.Button(wf_button_frame, text="Stop Workflow", command=self.stop_workflow)
        self.stop_button.pack(side=tk.LEFT, padx=5)
        self.stop_button.config(state=tk.DISABLED)
        
        # Workflow log
        ttk.Label(workflow_frame, text="Workflow Log:").pack(anchor=tk.W, pady=5)
        
        log_frame = ttk.Frame(workflow_frame)
        ...
        clear_log_frame = ttk.Frame(workflow_frame)
        clear_log_frame.pack(fill=tk.X, pady=5)
        
        ttk.Button(clear_log_frame, text="Clear Log", command=self.clear_log).pack(side=tk.RIGHT)
        ttk.Button(clear_log_frame, text="Save Log", command=self.save_log).pack(side=tk.RIGHT, padx=5)
        
        # Set up game selection event
        self.game_listbox.bind('<<ListboxSelect>>', self.on_game_select)
    
    def setup_template_tab(self):
        """Setup the template tools tab"""
        ...
        capture_frame = ttk.LabelFrame(template_frame, text="Template Capture", padding=10)
        capture_frame.pack(fill=tk.X, pady=10)
        
        ttk.Label(capture_frame, text="Capture UI elements for template matching:").pack(anchor=tk.W, pady=5)
        
        capture_button_frame = ttk.Frame(capture_frame)
        capture_button_frame.pack(fill=tk.X, pady=5)
        
        ttk.Button(capture_button_frame, text="Quick Screenshot", command=self.take_quick_screenshot).pack(side=tk.LEFT, padx=5)
        ttk.Button(capture_button_frame, text="Capture Template", command=self.capture_template).pack(side=tk.LEFT, padx=5)
        ttk.Button(capture_button_frame, text="Test Template", command=self.test_template).pack(side=tk.LEFT, padx=5)
        
        # Template management section
        manage_frame = ttk.LabelFrame(template_frame, text="Template Management", padding=10)
        manage_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        # Template list
        template_list_frame = ttk.Frame(manage_frame)
        template_list_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(template_list_frame, text="Available Templates:").pack(anchor=tk.W)
        ...
        template_button_frame = ttk.Frame(manage_frame)
        template_button_frame.pack(fill=tk.X, pady=5)
        
        ttk.Button(template_button_frame, text="Refresh List", command=self.refresh_templates).pack(side=tk.LEFT, padx=5)
        ttk.Button(template_button_frame, text="Open Folder", command=self.open_templates_folder).pack(side=tk.LEFT, padx=5)
        ttk.Button(template_button_frame, text="View Template", command=self.view_template).pack(side=tk.LEFT, padx=5)
    ...
    def setup_workflow_tab(self):
        ...
        workflow_frame = ttk.Frame(self.workflow_tab, padding=10)
        workflow_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(workflow_frame, text="Workflow Editor - Coming Soon!", font=("Arial", 16)).pack(pady=20)
        ttk.Label(workflow_frame, text="Edit YAML files directly for now.", font=("Arial", 12)).pack(pady=10)
        
        # Add button to open config folder
        ttk.Button(workflow_frame, text="Open Config Folder", command=self.open_config_folder).pack(pady=10)
